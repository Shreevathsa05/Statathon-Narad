...
export const updateSurvey = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    const allowedKeys = ["accessType", "allowedChannels", "status"];
    ...
    if (hasDisallowedUpdates) {
      throw new ApiError(
        400,
        "Active or completed surveys cannot have questions or config edited",
      );
    }
  }
...
export const deleteSurvey = asyncHandler(async (req, res) => {
  ...
  await Survey.findOneAndDelete({ surveyId: survey_id });
